...
override fun HTML.generateBody(@args) {
    body {
        ...
        div {
            unsafe {
                raw("<script src='https://www.google.com/recaptcha/api.js'></script><script>function giveReputation(token) { console.log(\"${user.id}\", token); SpicyMorenitta.net.perfectdreams.spicymorenitta.views.ReputationView.recaptchaCallback(\"${user.id}\", token); }</script>")
            }
        }
        div {
            unsafe {
                raw("<script src='${LorittaWebsite.WEBSITE_URL}v2/assets/js/app.js?v=${System.currentTimeMillis()}'></script>")
            }
        }
